...
<div class="wrap">
	...
	<form action="" method="post"><input type="submit" value="Update Queries" name="get"
	                                     class="button"></form>
	<table style="width:100%; margin:20px 0 20px 0; border:1px solid #ddd;">
		<tr style="background:#EFEFEF;">
			<td style="width:30%;font-size:13px; padding:5px 2px 5px 2px;"><a href="<?php echo
				$mdpWebmasterTools->full_url($_SERVER['REQUEST_URI']);?>&s=query">Query</a></td>
			<td style="width:10%;font-size:13px; padding:5px 2px 5px 2px;"><a href="<?php echo
				$mdpWebmasterTools->full_url($_SERVER['REQUEST_URI']);?>&s=impressions">Impressions</a>
			</td>
			<td style="width:10%;font-size:13px; padding:5px 2px 5px 2px;"><a href="<?php echo
				$mdpWebmasterTools->full_url($_SERVER['REQUEST_URI']);?>&s=impressions_change">Impressions
					Change</a></td>
			<td style="width:10%;font-size:13px; padding:5px 2px 5px 2px;"><a href="<?php echo
				$mdpWebmasterTools->full_url($_SERVER['REQUEST_URI']);?>&s=clicks">Clicks</a></td>
			<td style="width:10%;font-size:13px; padding:5px 2px 5px 2px;"><a href="<?php echo
				$mdpWebmasterTools->full_url($_SERVER['REQUEST_URI']);?>&s=clicks_change">Clicks
					Change</a></td>
			<td style="width:10%;font-size:13px; padding:5px 2px 5px 2px;"><a href="<?php echo
				$mdpWebmasterTools->full_url($_SERVER['REQUEST_URI']);?>&s=ctr">CTR</a></td>
			<td style="width:10%;font-size:13px; padding:5px 2px 5px 2px;"><a href="<?php echo
				$mdpWebmasterTools->full_url($_SERVER['REQUEST_URI']);?>&s=ctr_change">CTR Change</a>
			</td>
			<td style="width:10%;font-size:13px; padding:5px 2px 5px 2px;"><a href="<?php echo
				$mdpWebmasterTools->full_url($_SERVER['REQUEST_URI']);?>&s=avg_position">Avg.
					Position</a></td>
			<td style="width:10%;font-size:13px; padding:5px 2px 5px 2px;"><a href="<?php echo
				$mdpWebmasterTools->full_url($_SERVER['REQUEST_URI']);?>&s=avg_position_change">Avg.
		...
		if ($s = $_GET['s']) {
			...
			} elseif ($s == 'avg_position_change') {
				$selection = 'ORDER BY CAST(avg_position_change AS
				DECIMAL) DESC,
				avg_position_change + 0 DESC';
			} elseif ($s == 'impressions') {
				$selection = 'ORDER BY impressions + 0 DESC';
			} elseif ($s == 'impressions_change') {
				$selection = 'ORDER BY impressions_change + 0 DESC';
			} elseif ($s == 'query') {
